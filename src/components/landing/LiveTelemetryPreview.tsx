...
export function LiveTelemetryPreview() {
  const chartData = mockAqmsData;
  const currentConfig = aqmsConfig;
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-2 max-w-2xl">
            ...
            <p className="text-muted-foreground text-sm sm:text-base">
              Grafik data pengukuran kualitas udara 24 jam terakhir dari stasiun pemantauan.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 p-1 rounded-lg bg-muted border border-border/50 self-start md:self-auto">
            <Button
              variant="default"
              size="sm"
              className="text-xs font-semibold gap-1.5 cursor-default pointer-events-none"
            >
              <Wind className="w-3.5 h-3.5" />
              Stasiun Pemantauan Kualitas Udara (AQMS)
            </Button>
          </div>
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <CardDescription className="font-mono text-xs mt-0.5">
                  ID Perangkat: urn:uuid:DEV-AQMS-01
                </CardDescription>
              </div>
              <Link to="/login">
